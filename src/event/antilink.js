...
export const handleAntilink = async (m, sock, logger, isBotAdmins, isAdmins, isCreator) => {
    ...
    if (cmd === 'antilink') {
        ...
        if (!m.isGroup) {
            await sock.sendMessage(m.from, { text: 'Add Chucky to your group in order to use this command.' }, { quoted: m });
            return;
        }

        if (!isBotAdmins) {
            await sock.sendMessage(m.from, { text: 'Make Chucky an admin to manage the antilink feature.' }, { quoted: m });
            return;
        }
        ...
        if (action === 'on') {
            if (isAdmins) {
                ...
                await sock.sendMessage(m.from, { text: 'You have successfully enabled the antilink feature in this group chat.' }, { quoted: m });
            } else {
    ...
    if (antilinkSettings[m.from]) {
        if (m.body.match(/(chat.whatsapp.com\/)/gi)) {
            if (!isBotAdmins) {
                await sock.sendMessage(m.from, { text: `Make Chucky an admin in order to kick and delete links..` });
                return;
            }
